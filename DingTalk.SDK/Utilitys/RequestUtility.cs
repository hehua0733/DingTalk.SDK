...
namespace DingTalk.SDK.Utilitys.HttpUtility
{
    public static class RequestUtility
    {
        ...
        public static string PostFile(string url, string uploadfile, int timeOut = Config.TIME_OUT)
        {
            ...
            if (!string.IsNullOrEmpty(uploadfile))
            {
                ...
                string fileFormdataTemplate = "\r\n--" + boundary + "\r\nContent-Disposition: form-data;filelength={2}; name=\"{0}\"; filename=\"{1}\"\r\nContent-Type: application/octet-stream\r\n\r\n";
                ...
                {
                    //准备文件流
                    using (var fileStream = FileUtility.GetFileStream(uploadfile))
                    {
                        string formdata = null;
                        if (fileStream != null)
                        {
                            //存在文件
                            formdata = string.Format(fileFormdataTemplate, "media", /*fileName*/ Path.GetFileName(uploadfile), fileStream.Length);
                        }
...
#if DEBUG
                FileStream fs = new FileStream(@"d:\dddd.txt", FileMode.Create);
                postStream.WriteTo(fs);
#endif
        ...
        /// <returns></returns>
        public static string HttpPost(string url, CookieContainer cookieContainer = null, Stream postStream = null, string uploadfile = null, string refererUrl = null, Encoding encoding = null, int timeOut = Config.TIME_OUT, bool checkValidationResult = false)
        {
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "POST";
            request.Timeout = timeOut;
        ...
        public static string HttpPost(string url, string content, int timeOut)
        {
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "POST";
            request.Timeout = timeOut;
            ...
            request.KeepAlive = true;
            request.UserAgent = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/31.0.1650.57 Safari/537.36 DingTalk.SDK";
